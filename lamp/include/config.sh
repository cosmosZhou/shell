...
if [[ $(is_root) ]]; then
	...
	base=$(get_base)
	apache_location=$base/apache
	mysql_location=$base/mysql8
	mariadb_location=$base/mariadb
	php_location=$base/php
	openssl_location=$base/openssl

	#Install depends location
	depends_prefix=$base

	#Web root location
	web_root_dir=$base/gitlab
